        ...
        afterReply: async (_reply, context) => {
          ...
          await context.driver.sendContact(context.target, {
            ...
            vcard: [
              ...
              "TEL;TYPE=CELL:+15550000003",
              "END:VCARD",
            ].join("\n"),
          });
          const contactReply = await context.driver.waitForMessage({
            observedAfter: contactStartedAt,
            timeoutMs: 60_000,
            match: (message) =>
              message.fromPhoneE164 === context.sutPhoneE164 && message.text.includes(contactToken),
          });
          context.recordObservedMessage(contactReply);

          const stickerStartedAt = new Date();
          await context.driver.sendSticker(context.target, WHATSAPP_QA_ONE_PIXEL_WEBP, {
            mimetype: "image/webp",
          });
          const stickerReply = await context.driver.waitForMessage({
            observedAfter: stickerStartedAt,
            timeoutMs: 60_000,
            match: (message) =>
              message.fromPhoneE164 === context.sutPhoneE164 && message.text.includes(stickerToken),
          });
          context.recordObservedMessage(stickerReply);
          return "document, poll, location, contact, and sticker elicited replies";
        },
        configMode: "allowlist",
        expectReply: true,
        input:
